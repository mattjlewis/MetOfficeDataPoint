...
public class NasaIssFlyby {
	...
	private static final String DESCRIPTION_REGEX = String.format(
			"\\s*Date: (?<%s>.*) <br/>" + "\\s*Time: (?<%s>.*) <br/>"
					+ "\\s*Duration: (less than)?\\s*(?<%s>\\d*) minutes? <br/>"
					+ "\\s*Maximum Elevation: (?<%s>\\d*)° <br/>" + "\\s*Approach: (?<%s>\\d*)° above (?<%s>\\w*) <br/>"
					+ "\\s*Departure: (?<%s>\\d*)° above (?<%s>\\w*) <br/>.*",
			DATE_GROUP_ID, TIME_GROUP_ID, DURATION_GROUP_ID, MAX_ELEV_GROUP_ID, APPROACH_ANGLE_GROUP_ID,
			APPROACH_ELEV_GROUP_ID, DEPARTURE_ANGLE_GROUP_ID, DEPARTURE_ELEV_GROUP_ID);
	...
	private static final String TIME_FORMAT = "K:m a";
	private static final String LOCATION_FORMAT = "%s_%s_%s";

	public static void main(String[] args) {
		try {
			List<SatelliteFlyby> iss_passes = getFlybys("United Kingdom", "England", "Farnborough",
	...
	/**
	 * Based on the
	 * <a href="https://spotthestation.nasa.gov/sightings/index.cfm">NASA Sighting
	 * Opportunities RSS Feed</a>. Visit that page for valid values for country /
	 * region / city.
	 * 
	 ...
	 * @return
	 */
	public static List<SatelliteFlyby> getFlybys(String country, String region, String city, ZoneId zoneId)
			throws IOException {
		String location = String.format(LOCATION_FORMAT, country.replace(" ", "_"), region == null ? "None" : region,
				city);
			...
			for (Element item : items) {
				...
				if (!m.find()) {
					throw new IOException(
							"Error, the returned description element text didn't match the regular expression");
				}

				LocalDate date = date_formatter.parse(m.group(DATE_GROUP_ID), LocalDate::from);
				LocalTime time = time_formatter.parse(m.group(TIME_GROUP_ID).toLowerCase(), LocalTime::from);
				...
				iss_passes.add(new SatelliteFlyby(approach_date_time, approach_elev, approach_angle, max_elev,
