 ...
 * {@link DataProvidersManager DataProvidersManager}. This means TLE data may be
 * retrieved from many different storage media (local disk files, remote
 * servers, database ...).
 * </p>
 * <p>
...
public class TleSeries implements DataLoader {
	 ...
	 * <p>
	 * This constructor does not load any data by itself. Data must be loaded later
	 * on by calling one of the {@link #loadTLEData() loadTLEData()} method, the
	 * {@link #loadTLEData(int) loadTLEData(filterSatelliteNumber)} method or the
	 * {@link #loadTLEData(int, int, String) loadTLEData(filterLaunchYear,
	 * filterLaunchNumber, filterLaunchPiece)} method.
	 * <p>
	 * 
	 * @param supportedNames    regular expression for supported files names (if
	 *                          null, a default pattern matching files with a ".tle"
	 *                          extension will be used)
	 * @param ignoreNonTLELines if true, extra non-TLE lines are silently ignored,
	 *                          if false an exception will be generated when such
	 ...
	 * <p>
	 * The TLE data already loaded in the instance will be discarded and replaced by
	 * the newly loaded data.
	 * </p>
	 * <p>
	 * The filtering values will be automatically set to the first loaded satellite.
	 * This feature is useful when the satellite selection is already set up by
	 * either the instance configuration (supported file names) or by the
	 * {@link DataProvidersManager data providers manager} configuration and the
	 * local filtering feature provided here can be ignored.
	 * </p>
	 * 
	 * @see #loadTLEData(int)
	 * @see #loadTLEData(int, int, String)
	 ...
	 * 
	 * @return available satellite numbers file content is corrupted or no TLE data
	 *         is available
	 */
	 ...
	 * </p>
	 * <p>
	 * Calling this method with the satellite number set to a negative value, is
	 * equivalent to call {@link #loadTLEData()}.
	 * </p>
	 * 
	 ...
	 * </p>
	 * <p>
	 * Calling this method with either the launch year or the launch number set to a
	 * negative value, or the launch piece set to null or an empty string are all
	 * equivalent to call {@link #loadTLEData()}.
	 * </p>
	 * 
	 ...
	 * @see #loadTLEData(int)
	 */
	public void loadTLEData(final int launchYear, final int launchNumber, final String launchPiece) {
			...
			for (String line = r.readLine(); line != null; line = r.readLine()) {
				...
				if (pendingLine == null) {
					...
					if (filterSatelliteNumber < 0) {
						if ((filterLaunchYear < 0) || ((tle.getLaunchYear() == filterLaunchYear)
	...
	/**
	 * Get the extrapolated position and velocity from an initial date. For a good
	 * precision, this date should not be too far from the range :
	...
	public TLE getClosestTLE(final AbsoluteDate date) {
		...
		if ((previous != null) && (date.durationFrom(previous.getDate()) >= 0) && (next != null)
				&& (date.durationFrom(next.getDate()) <= 0)) {
